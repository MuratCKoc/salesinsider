...
function set_dropDown(data) {
  ...
  predicted_columns = columns.filter(function (str) {
...
function create_WordCloud() {
  function get_wordCloud_Data() {
    ...
    d3.json('/api/nodate').then((data) => {
      ...
      console.log(myarr.reverse().slice(0, 10))
      ...
      for (let i = 0; i < convArr.length; i++) {
        ...
        console.log("temp",temp_.size)
        temp_list.push(temp_)
      }
  ...
  console.log("function obj : ",myWords[0].word)
  // Yet another json reader
  // const clonedArr = JSON.parse(JSON.stringify(data));
  // console.log("jsonParse",clonedArr)
  ...
  console.log("myWords Words", myWords[0].word)
  // set the dimensions and margins of the graph
  var margin = {
      top: 10,
      right: 10,
